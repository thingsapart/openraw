...
void test_full_linear_pipeline() {
    ...
    float black_point = 0.f, white_point = 2000.f, exposure = 2.5f;
    Halide::Func normalized = pipeline_normalize_and_expose(raw_func, black_point, white_point, exposure, x, y);

    Halide::Func deinterleaved = pipeline_deinterleave(normalized, x, y, c);
    DemosaicBuilder demosaic_builder(deinterleaved, x, y, c, 0, 4, 4);
    ...
    Halide::Func color_corrected = pipeline_color_correct(demosaiced, matrix_func_3200, matrix_func_7000, 5000.0f, 0.0f, white_point, x, y, c, Halide::get_host_target(), false);
    
    Halide::Func to_uint16("to_uint16_final");
    to_uint16(x, y, c) = Halide::cast<uint16_t>(Halide::clamp(color_corrected(x, y, c) * 65535.f, 0.f, 65535.f));

    Halide::Buffer<uint16_t> output = to_uint16.realize({4, 4, 3});

    // 3. Manually calculate expected output
    float normalized_val = (1000.f - black_point) / (white_point - black_point) * exposure;
    ...
    float ir=normalized_val, ig=normalized_val, ib=normalized_val;
    float r_cc = m[0][3]/white_point + m[0][0]*ir + m[0][1]*ig + m[0][2]*ib;
    float g_cc = m[1][3]/white_point + m[1][0]*ir + m[1][1]*ig + m[1][2]*ib;
    float b_cc = m[2][3]/white_point + m[2][0]*ir + m[2][1]*ig + m[2][2]*ib;
    
    uint16_t r_final = static_cast<uint16_t>(std::max(0.f, std::min(65535.f, r_cc * 65535.f)));
    uint16_t g_final = static_cast<uint16_t>(std::max(0.f, std::min(65535.f, g_cc * 65535.f)));
    uint16_t b_final = static_cast<uint16_t>(std::max(0.f, std::min(65535.f, b_cc * 65535.f)));
