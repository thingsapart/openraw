...
void test_color_correct_behavior() {
    ...
    {
        auto input_buffer = make_constant_buffer<uint16_t>({2, 2, 3}, 0);
        Halide::Func input_func = buffer_to_func(input_buffer, "cc_black_input");
        Halide::Func corrected = pipeline_color_correct(input_func, matrix_func, matrix_func, 7000.f, 0.f, x, y, c, Halide::get_host_target(), false);
        Halide::Buffer<uint16_t> output = corrected.realize({2, 2, 3});
    ...
    {
        auto input_buffer = make_constant_buffer<uint16_t>({2, 2, 3}, 20);
        ...
        // This test now correctly checks the expected behavior of each channel.
    ...
    {
        // This value is high enough to guarantee the blue channel clips after matrix multiplication.
        auto input_buffer = make_constant_buffer<uint16_t>({2, 2, 3}, 15000);
