...
public:
    ...
    DemosaicRI(Halide::Func deinterleaved, Halide::Var x_full, Halide::Var y_full, Halide::Var c_full) : qx("ri_qx"), qy("ri_qy") {
        using namespace Halide;
        using namespace Halide::ConciseCasts;

        Func deinterleaved_i32("deinterleaved_i32_ri");
        ...
        Func g_final("g_final_ri");
        {
            ...
            g_tentative_r(qx, qy) = avg(deinterleaved_i32(qx, qy, 0), deinterleaved_i32(qx + 1, qy, 0),
                                        deinterleaved_i32(qx, qy, 3), deinterleaved_i32(qx, qy - 1, 3));
            Func g_tentative_b("g_tentative_b");
            g_tentative_b(qx, qy) = avg(deinterleaved_i32(qx, qy, 0), deinterleaved_i32(qx, qy + 1, 0),
                                        deinterleaved_i32(qx, qy, 3), deinterleaved_i32(qx - 1, qy, 3));

            // Calculate residuals where G is known
            Func g_residual_gr("g_residual_gr"), g_residual_gb("g_residual_gb");
            g_residual_gr(qx, qy) = deinterleaved_i32(qx, qy, 0) - avg(g_tentative_r(qx, qy), g_tentative_r(qx - 1, qy));
            g_residual_gb(qx, qy) = deinterleaved_i32(qx, qy, 3) - avg(g_tentative_b(qx, qy), g_tentative_b(qx, qy + 1));
            ...
            g_final(x_full, y_full) = mux(y_full % 2,
                                          {mux(x_full % 2, {deinterleaved_i32(x_full/2, y_full/2, 0), g_corrected_r(x_full/2, y_full/2)}),
                                           mux(x_full % 2, {g_corrected_b(x_full/2, y_full/2), deinterleaved_i32(x_full/2, y_full/2, 3)})});
        }
        intermediates.push_back(g_final);

        // --- Stage 2: R/B channels using G_final and color differences
        Func cd_r("cd_r"), cd_b("cd_b");
        cd_r(qx, qy) = deinterleaved_i32(qx, qy, 1) - deinterleaved_i32(qx, qy, 0);
        cd_b(qx, qy) = deinterleaved_i32(qx, qy, 2) - deinterleaved_i32(qx, qy, 3);
        ...
        Func r_final("r_final"), b_final("b_final");
        r_final(x_full, y_full) = g_final(x_full, y_full) + cd_r_interp(x_full, y_full);
        b_final(x_full, y_full) = g_final(x_full, y_full) + cd_b_interp(x_full, y_full);

        output = Func("demosaic_ri");
        output(x_full, y_full, c_full) = u16_sat(mux(c_full, {r_final(x_full, y_full), g_final(x_full, y_full), b_final(x_full, y_full)}));
    }
};
