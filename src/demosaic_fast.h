...
public:
    ...
    DemosaicFast(Halide::Func deinterleaved, Halide::Var x_full, Halide::Var y_full, Halide::Var c_full) : qx("fast_qx"), qy("fast_qy") {
        using namespace Halide;
        using namespace Halide::ConciseCasts;

        Func deinterleaved_i32("deinterleaved_i32_fast");
        ...
        Func r_at_gb("r_at_gb_fast"), b_at_gb("b_at_gb_fast");
        {
            ...
            Expr gb_val = deinterleaved_i32(qx, qy, 3);
            ...
            r_at_gr(qx, qy) = green_correction_r_gr + avg(deinterleaved_i32(qx - 1, qy, 1), r_val);

            Expr green_correction_b_gr = gr_val - avg(g_at_b(qx, qy), g_at_b(qx, qy - 1));
            b_at_gr(qx, qy) = green_correction_b_gr + avg(b_val, deinterleaved_i32(qx, qy - 1, 2));

            Expr green_correction_r_gb = gb_val - avg(g_at_r(qx, qy), g_at_r(qx, qy + 1));
            r_at_gb(qx, qy) = green_correction_r_gb + avg(r_val, deinterleaved_i32(qx, qy + 1, 1));
            
            Expr green_correction_b_gb = gb_val - avg(g_at_b(qx, qy), g_at_b(qx + 1, qy));
            b_at_gb(qx, qy) = green_correction_b_gb + avg(b_val, deinterleaved_i32(qx + 1, qy, 2));
        }
        ...
        Func r_at_b("r_at_b_fast"), b_at_r("b_at_r_fast");
        {
            ...
            rp_b += avg(deinterleaved_i32(qx, qy, 1), deinterleaved_i32(qx-1, qy+1, 1));
            Expr rpd_b = absd(deinterleaved_i32(qx, qy, 1), deinterleaved_i32(qx - 1, qy + 1, 1));

            Expr rn_b = g_at_b(qx, qy) - avg(g_at_r(qx-1, qy), g_at_r(qx, qy+1));
            rn_b += avg(deinterleaved_i32(qx-1, qy, 1), deinterleaved_i32(qx, qy+1, 1));
            Expr rnd_b = absd(deinterleaved_i32(qx - 1, qy, 1), deinterleaved_i32(qx, qy + 1, 1));
            r_at_b(qx, qy) = select(rpd_b < rnd_b, rp_b, rn_b);

            Expr bp_r = g_at_r(qx, qy) - avg(g_at_b(qx, qy), g_at_b(qx + 1, qy - 1));
            bp_r += avg(deinterleaved_i32(qx, qy, 2), deinterleaved_i32(qx+1, qy-1, 2));
            Expr bpd_r = absd(deinterleaved_i32(qx, qy, 2), deinterleaved_i32(qx+1, qy-1, 2));
            
            Expr bn_r = g_at_r(qx, qy) - avg(g_at_b(qx+1, qy), g_at_b(qx, qy-1));
            bn_r += avg(deinterleaved_i32(qx+1, qy, 2), deinterleaved_i32(qx, qy-1, 2));
            Expr bnd_r = absd(deinterleaved_i32(qx+1, qy, 2), deinterleaved_i32(qx, qy-1, 2));
        ...
        r_full(x_full, y_full) = mux(y_full % 2,
                                  {mux(x_full % 2, {r_at_gr(x_full/2, y_full/2), deinterleaved_i32(x_full/2, y_full/2, 1)}),
        ...
        b_full(x_full, y_full) = mux(y_full % 2,
                                  {mux(x_full % 2, {b_at_gr(x_full/2, y_full/2), b_at_r(x_full/2, y_full/2)}),
                                   mux(x_full % 2, {deinterleaved_i32(x_full/2, y_full/2, 2), b_at_gb(x_full/2, y_full/2)})});

        output = Func("demosaic_fast");
        output(x_full, y_full, c_full) = u16_sat(mux(c_full, {r_full(x_full, y_full), g_full(x_full, y_full), b_full(x_full, y_full)}));
    }
};
